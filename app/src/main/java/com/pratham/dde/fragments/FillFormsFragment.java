...
public class FillFormsFragment extends Fragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            ...
            if (BaseActivity.appDatabase != null)
...
//        getFormsfromDb();
        forms = BaseActivity.appDatabase.getDDE_FormsDao().getAllFormsByUserId("%," + userId + ",%");
        filterFormsWithEmptyQuestions();
        if (forms != null)
            updateUI();
        else
            Toast.makeText(getActivity(), "Problem in displaying forms for "+ userName, Toast.LENGTH_SHORT).show();
    }

    private void filterFormsWithEmptyQuestions() {
        List formsIdsToDelete = new ArrayList();
        for (int i = 0; i < forms.size(); i++) {
            List<DDE_Questions> questions = BaseActivity.appDatabase.getDDE_QuestionsDao().getFormIdWiseQuestions(String.valueOf(forms.get(i).getFormid()));
            if (questions.size() <= 0)
                formsIdsToDelete.add(forms.get(i));
        }

       forms.removeAll(formsIdsToDelete);
    }
...
//        }
    ...
    private void checkFormPassword(final String formId) {
        final int UserId = BaseActivity.appDatabase.getUserDao().getUserId(userName, password);
        List<DDE_Questions> formIdWiseQuestions = new ArrayList<>();
        formIdWiseQuestions = BaseActivity.appDatabase.getDDE_QuestionsDao().getFormIdWiseQuestions(formId);
        if (!formIdWiseQuestions.isEmpty()) {

            formPassword = BaseActivity.appDatabase.getDDE_FormsDao().getFormPassword(formId);
